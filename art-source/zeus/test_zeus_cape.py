"""Pure-Python gates for the authored Zeus cape; no client or Blender startup."""
import hashlib
import json
import math
import sys
...
class ZeusCapeTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.output = ROOT / 'prototype'
        cls.model = inspect(cls.output / 'models' / 'Zeus_Cape.bmd', True)
    ...
    def test_exported_skeleton_preserves_the_frozen_native_contract(self):
        ...
        recorded = audit['models'][AUDIT_KEY]
        self.assertEqual(recorded['member'], CAPE_MEMBER)
        self.assertEqual(recorded['sha256'], native['sha256'])
        self.assertEqual(recorded['bones'][0]['name'], 'collar')
        self.assertEqual(recorded['triangles'], 192)
    ...
    def test_triangle_budget_holds_and_matches_report(self):
        ...
        self.assertEqual((budget['low'], budget['high']), (6000, 10000))
    ...
    def test_generation_and_saved_scene_roundtrip(self):
        ...
        blend_hash = hashlib.sha256((self.output / 'zeus-cape.blend').read_bytes()).hexdigest()
    ...
    def test_preview_cannot_be_mistaken_for_an_installable_package(self):
        ...
        self.assertEqual(self.build['native_contract']['audit_sha256'],
                         hashlib.sha256((ROOT / 'native-reference-audit.json').read_bytes()).hexdigest())
    ...
    def test_equipped_preview_follows_the_player_rig(self):
        ...
        expected = [f'Zeus_Cape_{view}.png' for view in ('front', 'side', 'back')]
        expected += ['Zeus_CapeSet_equipped_front.png', 'Zeus_CapeSet_equipped_back.png']
    ...
    def test_cloth_contract_study_documents_the_runtime_anchors(self):
        ...
        for anchor in ('MODEL_CAPE_OF_EMPEROR', 'LinkBone = 19', 'CPhysicsCloth',
                       'dl_redwings', 'BITMAP_ROBE', 'RenderLinkObject',
                       'RATE_SHORT_SHOULDER', 'PCT_CURVED', 'collar',
                       'Data/Item/DarkLordRobe02.bmd'):
            self.assertIn(anchor, study, anchor)

    def test_study_documents_the_duel_master_class_gate(self):
        """The class difference vs the Poseidon study: bCloak is not open to DM."""
        study = (ROOT / 'cape-cloth-contract.md').read_text(encoding='utf-8')
        for needle in ('CLASS_DUELMASTER', 'CLASS_DARK_LORD', 'GetBaseClass',
        ...
        self.assertIn('case CLASS_DUELMASTER:\n        return CLASS_DARK;', manager)
