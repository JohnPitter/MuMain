...
from zeus_cape_rig import (cape_bind, decoded_skinned_triangles, digest,
...
def main():
    ...
    scene_file = output / 'zeus-cape.blend'
    rig = load_cape_rig()
    bpy.ops.wm.open_mainfile(filepath=str(scene_file))
    collection = bpy.data.collections['Zeus Cape']
    ...
    (output / 'cape-saved-roundtrip-report.json').write_text(json.dumps(report, indent=2),
                                                             encoding='utf-8')
    print('ZEUS_CAPE_SAVED_ROUNDTRIP_VERIFIED', json.dumps(report), flush=True)
