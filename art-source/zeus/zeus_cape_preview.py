...
def studio(target=CAPE_TARGET_Z):
    ...
    for name, location, energy, color, size in (
            ('Key', (-95, -150, target + 30), 700000, (1, .9, .74), 120),
            ('Fill', (100, -80, target - 85), 380000, (.6, .78, 1), 110),
            ('Rim', (45, 110, target + 45), 520000, (.72, .82, 1), 130)):
    ...
    camera_data = bpy.data.cameras.new('Zeus cape inspection camera')
...
def frame_cape(scene, view):
    ...
    scene.camera.location = (430 * math.sin(angle), -430 * math.cos(angle), CAPE_TARGET_Z)
...
def render_cape(scene, destination):
    for view in VIEWS:
        ...
        scene.render.filepath = str(destination / f'Zeus_Cape_{view}.png')
