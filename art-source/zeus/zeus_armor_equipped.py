"""Pose the authored Zeus pieces on the real player rig for fit inspection.
...
Run: blender --background --python-exit-code 1 --python zeus_armor_equipped.py
"""
import json
import sys
...
from zeus_armor_rig import USED_BONE_CONTRACT, load_player_rig
...
def main():
    ...
    models = {name: inspect(OUTPUT / 'models' / f'Zeus_{name}.bmd', True)
    ...
    for name, clip, frame, camera_location, target in POSES:
        ...
        scene.render.filepath = str(OUTPUT / 'renders' / f'Zeus_ArmorSet_{name}.png')
        bpy.ops.render.render(write_still=True)
        poses.append(dict(render=f'Zeus_ArmorSet_{name}.png', clip=clip,
        ...
        print('ZEUS_EQUIPPED_POSE', json.dumps(poses[-1]), flush=True)
    ...
    (OUTPUT / 'armor-equipped-review-report.json').write_text(
        json.dumps(report, indent=2), encoding='utf-8')
    print('ZEUS_EQUIPPED_VERIFIED',
