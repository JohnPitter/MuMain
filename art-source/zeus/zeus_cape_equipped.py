...
zeus_cape_rig (BoneTransform[19] . M1 . M2 . bone0), next to a neutral
mannequin built from generic primitives. Rigid ferragem follows bone 19 in
every pose; the fabric stays procedural cloth owned by the runtime (and the
Duel Master still needs its own cloth case - cape-cloth-contract.md).
Run: blender --background --python-exit-code 1 --python zeus_cape_equipped.py
"""
import json
import sys
import tempfile
...
from zeus_armor_equipped import clear_scene, mannequin, mannequin_material
from zeus_cape_materials import create_cape_materials
from zeus_cape_preview import point_at, studio
from zeus_cape_rig import (ARCHIVE, LINK_BONE, PLAYER_MEMBER, link_matrix,
                           load_cape_rig)
...
POSES = (('equipped_front', 'PLAYER_STOP_SWORD', 0, (0, -620, 150), (0, 0, 118)),
         ('equipped_back', 'PLAYER_WALK_SWORD', 3, (55, 620, 150), (0, 0, 118)))
ORTHO_SCALE = 430
SPAN_GATE = 135.0
RIGIDITY_GATE = 0.75
FOLLOW_GATE = 3.0
...
def load_player():
    with zipfile.ZipFile(ARCHIVE) as archive, tempfile.TemporaryDirectory(prefix='zeus-cape-eq-') as temp:
...
def cloth_proxy(player_world, bone0):
    ...
    physics sag is not simulated; the fall is an envelope guess. The Zeus
    cape reuses this grid layout as its cloth precedent. Preview only;
    ...
    shader.inputs['Base Color'].default_value = (.04, .09, .25, 1)
...
def main():
    ...
    model = inspect(OUTPUT / 'models' / 'Zeus_Cape.bmd', True)
    ...
    for name, clip, frame, camera_location, target in POSES:
        ...
        scene.render.filepath = str(OUTPUT / 'renders' / f'Zeus_CapeSet_{name}.png')
        bpy.ops.render.render(write_still=True)
        poses.append(dict(render=f'Zeus_CapeSet_{name}.png', clip=clip,
                          action=clips[clip], frame=frame, view=name.split('_')[1]))
        print('ZEUS_CAPE_EQUIPPED_POSE', json.dumps(poses[-1]), flush=True)
    ...
    (OUTPUT / 'cape-equipped-review-report.json').write_text(json.dumps(report, indent=2),
                                                             encoding='utf-8')
    print('ZEUS_CAPE_EQUIPPED_VERIFIED', json.dumps(report['span_probes']), flush=True)
