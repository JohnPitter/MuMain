"""Build the original Poseidon black mount on the audited DarkHorse rig.

Regenerable Python source skinned with single-influence bindings to the
frozen 60-bone DarkHorse skeleton, diagnostic BMD export that preserves the
complete seven-action native animation block (never installable, atlases
deliberately absent), bit-exact skeleton proof, BMD roundtrip and front/
side/back plus detail inspection renders.
Run: blender --background --python-exit-code 1 --python build_poseidon_mount.py [-- --skip-render]
"""
import argparse
import json
import sys
...
from poseidon_mount_rig import ACTION_EVIDENCE, ACTION_NAMES, MODEL_NAME, load_native_rig
from poseidon_mount_shapes import mount
...
# The mount is the set's largest model: the budget band follows the task
# envelope (15k-30k justified per part), landing near the measured density of
# the authored armor lots scaled to a full quadruped with bard and caparison.
TRIANGLE_BUDGET = (15000, 30000)
BUDGET_JUSTIFICATION = {
    'barrel_rump_chest': 'Four banded quarters of the body; the largest single mass (bones 17-20).',
    'flank_bard_caparison': 'Scalloped bard panels and riding cloth per flank with rune jewels.',
    'breastplate': 'Triple-point sea breastplate with gold blades and ocean gem.',
    'neck_crest': 'Two neck bands, withers/mane crest fins and gold scale rows.',
    'head_bard': 'Head tube, wrapping chamfron, chin medallion and ear fins.',
    'saddle_girth': 'Rider saddle, cantle, pommel arc and girth on the native saddle band.',
    'tail': 'Three armored tail links with gold rings and a bladed tip.',
    'legs': 'Four barded legs: pauldrons, haunches, sleeves, shoes with fetlock crowns.',
}
...
VIEWS = (('front', 0, (0, 6, 118), 240), ('side', 72, (0, -20, 120), 330),
         ('back', 180, (0, -20, 118), 240), ('detail_head', -38, (0, -100, 218), 120),
         ('detail_saddle', 148, (0, -5, 172), 130))
# Every authored object belongs to exactly one budget part by construction;
# prefixes are matched against object names and must stay exhaustive.
PART_PREFIXES = (
    ('barrel_rump_chest', ('Mount_rump', 'Mount_barrel', 'Mount_mid_barrel', 'Mount_chest',
                           'Mount_belly', 'Mount_hip', 'Mount_crupper')),
    ('flank_bard_caparison', ('Mount_flank', 'Mount_caparison')),
    ('breastplate', ('Mount_breast',)),
    ('neck_crest', ('Mount_neck', 'Mount_upper_neck', 'Mount_withers', 'Mount_mane')),
    ('head_bard', ('Mount_head', 'Mount_chamfron', 'Mount_chin', 'Mount_ear', 'Mount_bridle',
                   'Mount_brow', 'Mount_bit', 'Mount_nose')),
    ('saddle_girth', ('Mount_saddle', 'Mount_cantle', 'Mount_pommel', 'Mount_girth')),
    ('tail', ('Mount_tail',)),
    ('legs', ('Mount_front', 'Mount_rear', 'Mount_pauldron', 'Mount_haunch')),
)
...
def triangle_area_report(objects):
    ...
    if minimum < MIN_TRIANGLE_AREA:
        raise ValueError(f'Degenerate triangle in authored mount: {minimum}')
    return minimum
...
def main():
    ...
    item = collection(COLLECTION, lambda: mount(create_materials()))
    ...
    if not TRIANGLE_BUDGET[0] <= report['triangles'] <= TRIANGLE_BUDGET[1]:
        raise ValueError(f'Mount outside its triangle budget: {report["triangles"]}')
    ...
    if model['bones'] != native['bones'] or \
        ...
        raise ValueError('Exported mount skeleton diverges from the audited DarkHorse contract')
    ...
    print('POSEIDON_MOUNT_AUTHORED', json.dumps(report), flush=True)
    studio(120)
    if not args.skip_render:
        render_views(output)
    bpy.ops.wm.save_as_mainfile(filepath=str(output / 'poseidon-mount.blend'))
    ...
    full = dict(status='AUTHORED_MOUNT_PROTOTYPE_PREVIEW_ONLY',
                renderer='Blender Cycles / AgX, not MU runtime',
                design_reference='art-source/poseidon/design-spec.md',
                model_origin='Original Poseidon geometry on the audited native DarkHorse skeleton; '
                             'single-influence rig binding, no native mesh reuse, full 7-action '
                ...
                design_decisions={'mounted_lance': 'not modeled: the design-spec horse row defines '
                                                   'no mounted weapon; set weapons are the hand-held '
                                                   'Tridente/Cetro',
                                  'eagle_contract': 'see poseidon_eagle_rig.py: DarkSpirit pet rig '
                                                    'per design-spec, not a shoulder ornament'},
                triangle_budget=list(TRIANGLE_BUDGET),
                budget_justification=BUDGET_JUSTIFICATION,
                shared_source_sha256=before,
                blend_sha256=digest(output / 'poseidon-mount.blend'),
                pieces={MODEL_NAME: report})
    (output / 'mount-build-report.json').write_text(json.dumps(full, indent=2), encoding='utf-8')
    print('POSEIDON_MOUNT_VERIFIED', json.dumps(
