"""Reopen the saved armor scene and independently decode the exported BMD payloads."""
import json
import sys
...
def main():
    ...
    scene_file = output / 'poseidon-armor.blend'
    rig = load_native_rig()
    ...
    (output / 'armor-saved-roundtrip-report.json').write_text(json.dumps(report, indent=2),
                                                             encoding='utf-8')
    print('POSEIDON_ARMOR_SAVED_ROUNDTRIP_VERIFIED', json.dumps(report), flush=True)
