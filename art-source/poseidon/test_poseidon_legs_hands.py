"""Pure-Python gates for the authored Poseidon pants and gloves; no Blender startup."""
import hashlib
import json
import math
import sys
...
PIECES = ('Pant', 'Glove')
NATIVE_MEMBERS = {'Pant': 'Data/Player/PantClass305.bmd',
                  'Glove': 'Data/Player/GloveClass305.bmd'}
# Measured native vertex bindings (see poseidon_legs_hands_rig.py): the pants
# hang on pelvis, thighs, calves, spine waist and the Bone02 skirt bone; the
# gloves use forearm, hand and Finger0 per arm - no renumbering, nothing else.
USED_BONES = {'Pant': (2, 3, 4, 10, 11, 17, 44), 'Glove': (27, 28, 29, 36, 37, 38)}
ATLASES = {'Poseidon_Black.jpg', 'Poseidon_Gold.jpg', 'Poseidon_Blue.jpg'}
ROUNDTRIP_GATE = 1e-6


class PoseidonLegsHandsTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.output = ROOT / 'prototype'
        cls.models = {name: inspect(cls.output / 'models' / f'Poseidon_{name}.bmd', True)
                      for name in PIECES}
        cls.build = json.loads((cls.output / 'legs-hands-build-report.json').read_text())
    ...
    def test_generation_and_saved_scene_roundtrip(self):
        saved = json.loads((self.output / 'legs-hands-saved-roundtrip-report.json').read_text())
        blend_hash = hashlib.sha256(
            (self.output / 'poseidon-legs-hands.blend').read_bytes()).hexdigest()
        self.assertEqual(saved['saved_blend_sha256'], blend_hash)
        self.assertEqual(self.build['blend_sha256'], blend_hash)
        for name, model in self.models.items():
            # Observed band: weapons 5.4e-7..6.9e-7, lote 2 armor 6.2e-7..8.5e-7;
            # legs/hands measure 5.9e-7..8.9e-7 - bone-local float32 spacing again.
    ...
    def test_preview_cannot_be_mistaken_for_an_installable_package(self):
        self.assertEqual(self.build['status'], 'AUTHORED_LEGS_HANDS_PROTOTYPE_PREVIEW_ONLY')
    ...
    def test_equipped_preview_follows_the_player_rig(self):
        equipped = json.loads((self.output / 'legs-hands-equipped-review-report.json').read_text())
        ...
        shifts = equipped['anchor_gates']['pose_shifts']
        for piece in PIECES:
            self.assertGreaterEqual(shifts[piece], equipped['anchor_gates']['pose_shift_min'], piece)
        renders = self.output / 'renders'
        expected = [f'Poseidon_{piece}_{view}.png' for piece in PIECES for view in ('front', 'side', 'back')]
        expected += ['Poseidon_LegsHands_equipped_front.png', 'Poseidon_LegsHands_equipped_side.png']
