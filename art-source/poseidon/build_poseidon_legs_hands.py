"""Build original Poseidon pants and gloves prototypes on the audited native rig.

Same pipeline as build_poseidon_armor.py: regenerable Python sources skinned
with single-influence bindings to the frozen Pant/GloveClass305 skeletons,
diagnostic BMD export (never installable, atlases deliberately absent), BMD
roundtrip proof and front/side/back inspection renders.
Run: blender --background --python-exit-code 1 --python build_poseidon_legs_hands.py [-- --skip-render]
"""
import argparse
import json
import sys
...
from poseidon_legs_hands_preview import render_piece
from poseidon_legs_hands_rig import load_native_rig, USED_BONE_CONTRACT
from poseidon_legs_hands_shapes import gloves, pants
from poseidon_materials import PALETTE, create_materials
from verify_prop_roundtrip import compare

PIECES = (('Pant', pants), ('Glove', gloves))
# Lote-2 proportion: boots measured 4192 and armor 7842 triangles. Pants cover
# both full legs, waist, fauld and a free skirt on Bone02; gloves are complete
# pairs with articulated claws but the smallest worn surface - hence symmetric,
# slightly tighter bands around the measured densities.
TRIANGLE_BUDGETS = {'Pant': (4000, 6600), 'Glove': (2600, 5200)}
BUDGET_JUSTIFICATION = (
    'Proportional to the measured lote 2 pieces (boot 4192, armor 7842, helm 3484 '
    'triangles): pants dress both legs plus waist, fauld and free-hanging skirt, '
    'landing between boot and armor; gloves are complete pairs of the smallest '
    'worn pieces with segmented claw fingers, just above the helm floor.')
...
def triangle_area_report(objects):
    ...
    if minimum < MIN_TRIANGLE_AREA:
        raise ValueError(f'Degenerate triangle in authored legs/hands: {minimum}')
    return minimum
...
def build_pieces(materials, rig, output):
    ...
    for name, builder in PIECES:
        ...
        reports[name] = report
        print('POSEIDON_LEGS_HANDS_AUTHORED', json.dumps(report), flush=True)
...
def write_reports(output, reports, rig, shared_hashes):
    report = dict(status='AUTHORED_LEGS_HANDS_PROTOTYPE_PREVIEW_ONLY',
                  renderer='Blender Cycles / AgX, not MU runtime',
                  design_reference='art-source/poseidon/design-spec.md',
                  model_origin='Original Poseidon geometry on the audited native Class305 skeletons; '
                  ...
                  blend_sha256=digest(output / 'poseidon-legs-hands.blend'),
                  pieces=reports)
    (output / 'legs-hands-build-report.json').write_text(json.dumps(report, indent=2), encoding='utf-8')
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(output / 'poseidon-legs-hands.blend'))
    ...
    print('POSEIDON_LEGS_HANDS_VERIFIED', json.dumps(
