"""Extract the UV region layout of the five Poseidon models.
...
documents which model regions sample which definitive atlas.

Run: blender --background --python-exit-code 1 --python inspect_poseidon_uvs.py
"""
import json
import sys
...
SCENES = (('Trident', 'Scepter'), 'poseidon-weapons.blend'), \
         (('Helm', 'Armor', 'Boot'), 'poseidon-armor.blend')
...
def main():
    ...
    report = dict(status='UV_LAYOUT_PROOF_FOR_DEFINITIVE_ATLASES',
                  ...
                  models=regions)
    for name in regions:
        model = inspect(ROOT / 'prototype' / 'models' / f'Poseidon_{name}.bmd', True)
    ...
    print('POSEIDON_UV_REGIONS_VERIFIED', json.dumps(
