"""Build original Poseidon armor prototypes on the audited native rig.

Generates Helm, Armor and Boot from regenerable Python sources, skins them with
single-influence bindings to the frozen Class305 skeletons, exports diagnostic
BMDs (never installable, atlas files deliberately absent), verifies the BMD
roundtrip against the editable scene and renders front/side/back inspections.
Run: blender --background --python-exit-code 1 --python build_poseidon_armor.py [-- --skip-render]
"""
import argparse
import hashlib
import json
...
from poseidon_armor_preview import render_piece, studio
...
TRIANGLE_BUDGETS = {'Helm': (2200, 6200), 'Armor': (3200, 8200), 'Boot': (2600, 6600)}
...
def triangle_area_report(objects):
    ...
    if minimum < MIN_TRIANGLE_AREA:
        raise ValueError(f'Degenerate triangle in authored armor: {minimum}')
    return minimum
...
def build_pieces(materials, rig, output):
    ...
    for name, builder in PIECES:
        ...
        reports[name] = report
        print('POSEIDON_ARMOR_AUTHORED', json.dumps(report), flush=True)
...
def write_reports(output, reports, rig, shared_hashes):
    ...
    report = dict(status='AUTHORED_ARMOR_PROTOTYPE_PREVIEW_ONLY',
                  ...
                  blend_sha256=digest(output / 'poseidon-armor.blend'),
                  pieces=reports)
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(output / 'poseidon-armor.blend'))
    ...
    print('POSEIDON_ARMOR_VERIFIED', json.dumps(
