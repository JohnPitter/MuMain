...
The native cape (Data/Item/DarkLordRobe02.bmd) carries one bone ("collar")
and one pose; the visible fabric is procedural cloth created by the client
(see cape-cloth-contract.md). The rigid ferragem is linked to player bone 19
in ZzzCharacter.cpp: RenderLinkObject composes

    M1 = AngleMatrix(0, 90, 0) + T(-47, -7, 0)      # cape branch (~line 6841)
...
space and later posed exactly where the runtime would put it. No native mesh
...
def load_cape_rig():
    ...
    with zipfile.ZipFile(ARCHIVE) as archive, tempfile.TemporaryDirectory(prefix='poseidon-cape-') as temp:
