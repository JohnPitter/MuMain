"""Reopen the saved eagle scene and independently decode the exported BMD."""
import json
import sys
...
from poseidon_eagle_rig import MODEL_NAME, USED_BONE_CONTRACT, load_native_rig
...
def main():
    ...
    scene_file = output / 'poseidon-eagle.blend'
    rig = load_native_rig()
    ...
    if model['bones'] != native['bones'] or \
        ...
        raise ValueError('Saved-scene eagle skeleton diverges from the audited DarkHorse contract')
    used = sorted({bone for mesh in model['meshes'] for bone in mesh['used_bones']})
    if tuple(used) != USED_BONE_CONTRACT:
        raise ValueError(f'Eagle used bones {used} violate the native contract')
    report = dict(saved_blend_sha256=digest(scene_file),
                  models={MODEL_NAME: dict(
                      ...
                      used_bones=used)}) 
    (output / 'eagle-saved-roundtrip-report.json').write_text(json.dumps(report, indent=2),
                                                              encoding='utf-8')
    print('POSEIDON_EAGLE_SAVED_ROUNDTRIP_VERIFIED', json.dumps(report), flush=True)
