...
def staff_halo(m):
    ...
    for i in range(12):
        ...
        feather('Staff / solar ray', [(x * .83, 0, 86 + (z - 86) * .83),
                (x, 0, z), (end[0], 0, end[2] - 1), end], (1.15, 0.55), m['Trim'])
...
def staff(m):
    ...
    for side in (-1, 1):
        tendril('Staff / rising split prong', [(side * 1.2, -0.5, 52),
                (side * 27, 0, 74), (side * 4, 0, 99), (side * 9, 0, 121)], 1.1, m['Trim'])
        feather('Staff / ivory lance', [(side * 3, -0.9, 63), (side * 13, -1, 81),
                (side * 8, -1, 99), (side * 11, -1, 113)], (2.4, 0.7), m['Base'])
        tendril('Staff / scrolling bridge', [(side * 2, -1.5, 76),
                (side * 16, -2, 78), (side * 14, -2, 97), (0, -1, 100)], 0.6, m['Trim'])
    feather('Staff / long crown spear', [(0, 0, 95), (0, 0, 103),
            (0, 0, 115), (0, 0, 128)], (1.85, 0.8), m['Trim'])
    diamond = [(0, 98), (5.5, 86), (0, 74), (-5.5, 86)]
    rim('Staff / gemstone bezel', diamond, -3.0, m['Trim'], 0.65)
        ...
        for i in range(4):
            ...
            outline = [(side * (shield_width(height) + inset), height)
                       for height, inset in ((z + 7, -3), (z + 2, .5),
                                             (z - 13, -2), (z - 2, -7))]
            plate('Shield / gilded side scales', outline, (-3, 1.3), m['Trim'])
        for i in range(3):
            z = 27 - i * 13
            tendril('Shield / openwork acanthus', [(side * 6, -10, z),
                    (side * 16, -9, z + 12), (side * 23, -8, z + 3),
                    (side * 14, -8, z - 2)], .5, m['Trim'])
            gem('Shield / filigree clasp', (side * 12, -9, z), (1.3, .5, 2.5), m['Trim'])
        feather('Shield / crown leaf', [(side * 5, -6, 35), (side * 16, -6, 39),
                (side * 12, -4, 53), (side * 13, -2, 62)], (3.1, .8), m['Trim'])
...
def jewel_crest(m, prefix, factor=1):
    ...
    plate(prefix + ' / ivory seat', outline, (-1.5 * factor, factor), m['Base'])
    rim(prefix + ' / gold bezel', outline, -2.2 * factor, m['Trim'], .45 * factor)
    ...
    for side in (-1, 1):
        for i in range(3):
            ...
            feather(prefix + ' / feather crown', controls, (factor, .4 * factor), m['Trim'])
        for i in range(3):
            z = -5 + i * 4
            controls = [(side * 3, -3, z), (side * 8, -3, z + 2),
                        (side * 5, -3, z + 5), (side * 3.5, -3, z + 2)]
            controls = [tuple(c * factor for c in point) for point in controls]
            tendril(prefix + ' / pierced leafwork', controls, .3 * factor, m['Trim'])
    for z in (-7, 7):
        gem(prefix + ' / star clasp', (0, -3 * factor, z * factor),
            (1.0 * factor, .5 * factor, 1.8 * factor), m['Trim'])


def pendant(m):
    jewel_crest(m, 'Pendant')
    gem('Pendant / hanging gold drop', (0, -1.5, -11.7), (1.3, .7, 2.7), m['Trim'])
    ellipse('Pendant / bail', (0, 0, 11.3), ((1.5, 0, 0), (0, 0, 2.2)), m['Base'], .4, 24)
    for side in (-1, 1):
        for i in range(11):
            t = i / 10
...
def ring(m):
    ellipse('Ring / outer band', (0, 4, -3), ((7.5, 0, 0), (0, 0, 7.5)), m['Base'], 1.3)
