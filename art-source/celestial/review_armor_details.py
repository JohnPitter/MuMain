...
VIEWS = (('front', False, (.12, -1, .04)), ('rear', False, (-.12, 1, .04)),
         ('side', False, (1, .2, .04)), ('helm-front', True, (.12, -1, .1)),
         ('helm-rear', True, (-.12, 1, .1)))
...
def main():
    ...
    for name, helmet_only, direction in VIEWS:
        scene = setup_scene()
        objects = import_surface(head, pose, palette)
    ...
    report = dict(status='decoded armor review; not ingame or artistic acceptance',
                  ...
                  player_sha256=player['sha256'], head_sha256=head['sha256'], action=4, frame=0,
