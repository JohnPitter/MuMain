...
def greave(palette, side):
    ...
    segment_shell('Boots / tapered greave', ((center, 0, 13), (center, -4.9, 63)),
                  [(0, 5.5, 6), (.25, 7, 7), (.65, 9, 9), (1, 9, 9)], palette['Base'])
    outline = [(center, 75), (side * 21, 59), (side * 16, 30),
               (side * 13, 15), (side * 7, 15), (side * 3, 57)]
    plate('Boots / pointed shin plate', outline, (-10, 3), palette['Base'])
    rim('Boots / shin gold frame', outline, -10.5, palette['Trim'], .7)
...
def posterior_greave(palette, side):
    ...
    for direction in (-1, 1):
        ...
        tendril('Boots / posterior calf filigree', [(center + direction * 3, -7, 18),
                ...
                (center + direction * 6, -3.3, 61)], .42, palette['Trim'])
    ...
    rim('Boots / rear sapphire bezel', diamond, -6.8, palette['Trim'], .35)
...
def ankle_collar(palette, side):
    ...
    for direction in (-1, 1):
        ...
        tube('Boots / articulated ankle edging', path, .55, palette['Trim'], 6)
...
def foot_shell(palette, side):
    ...
    obj = mesh('Boots / pointed sabaton', vertices, faces, palette['Base'])
...
def sole(palette, side):
    ...
    obj = mesh('Boots / defined gilded sole', vertices, faces, palette['Trim'])
...
def heel_counter(palette, side):
    ...
    for direction in (-1, 1):
        tendril('Boots / heel-to-instep scroll', [(center + direction * 4.3, 7.5, 7),
                ...
                (center + direction * 4.4, -14, 8)], .42, palette['Trim'])
...
def dorsal_scales(palette, side):
    ...
    for index, (y, width, height) in enumerate(((-18, 3.7, 9.5), (-12, 5.4, 12.2),
        ...
        obj = mesh('Boots / articulated instep scale', top, faces, palette['Base'])
