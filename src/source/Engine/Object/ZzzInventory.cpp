...
void RenderObjectScreen(int Type, int ItemLevel, int excellentFlags, int ancientDiscriminator, vec3_t Target, int Select, bool PickUp)
{
    ...
    // Classic UI compensation: the per-model ladder above is the Webzen-tuned
    // inventory scale, but the seven UI jewels read oversized, so shrink them
    // uniformly (Jewel of Chaos lands in the wing catch-all and needs this too).
