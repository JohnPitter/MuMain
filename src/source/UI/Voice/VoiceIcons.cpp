...
namespace
{
    // Frame sizes must match MIC_* / SND_* in tools/make_hud_button_icons.py.
    // The generator leaves the glyph one row short of its frame on purpose:
    // DrawFrame samples a vertically stacked sheet with GL_LINEAR, so art that
    // touched the frame boundary would bleed the muted frame into the active
    // one along the seam.
