...
namespace UI::HUD::MiniMap
{
    ...
    void Render()
    {
        ...
        // Run the TAB map's own draw (CNewUIMiniMap::Render) into this window:
        // same texture, same draw size (follows the TAB zoom), same hero-texel
        // anchor numbers, same 45-degree RenderBitRotate/RenderPointRotate
        // calls — only the render pivot (this window's center) and the scissor
        // differ. Same code = same result: 1:1 texel sharpness and the hero
        // exactly where TAB puts it, map edges included (TAB does not clamp;
        // the frame backdrop shows past the sheet edge, like TAB).
        const POINT metrics = g_pNewUIMiniMap->GetTabDrawMetrics();
        const float drawW = static_cast<float>(metrics.x);
        const float drawH = static_cast<float>(metrics.y);
        const float tx = (Hero->PositionY / 256.f) * drawW;
        const float ty = (Hero->PositionX / 256.f) * drawH;
        const float viewCenterX = mapX + (kMapSize / 2.f);
        const float viewCenterY = mapY + (kMapSize / 2.f);

        EnableAlphaTest();
        glColor4f(1.f, 1.f, 1.f, 1.f);
        RenderBitRotate(SEASON3B::CNewUIMiniMap::IMAGE_MINIMAP_INTERFACE,
            drawW - tx, drawH - ty, drawW, drawH, 45.f, viewCenterX, viewCenterY, true);

        if (PartyNumber > 0)
        {
            g_pPartyManager->SyncLivePartyPositions();
            g_pPartyManager->RequestPartyListIfDue();
            constexpr float kPartyMarkerSize = 12.f;
            for (int i = 0; i < PartyNumber; ++i)
            {
                ...
                const float memberTx = (member->y / 256.f) * drawW;
                const float memberTy = (member->x / 256.f) * drawH;
                glColor4f(0.25f, 0.85f, 1.f, 1.f);
                RenderPointRotate(SEASON3B::CNewUIMiniMap::IMAGE_MINIMAP_INTERFACE + 3,
                    memberTx, memberTy, kPartyMarkerSize, kPartyMarkerSize,
                    drawW - tx, drawH - ty, drawW, drawH, 45.f, 0.f,
                    17.5f / 32.f, 17.5f / 32.f, -1, viewCenterX, viewCenterY, true);
            }
        }

        // Hero marker: the hero is the pivot, exactly like TAB centers it.
        glColor4f(0.1f, 0.1f, 0.1f, 1.f);
        RenderColor(viewCenterX - 2.5f, viewCenterY - 2.5f, 5.f, 5.f);
        glColor4f(1.f, 0.9f, 0.2f, 1.f);
        RenderColor(viewCenterX - 2.f, viewCenterY - 2.f, 4.f, 4.f);
        EndRenderColor();
        DisableScissorTest();
