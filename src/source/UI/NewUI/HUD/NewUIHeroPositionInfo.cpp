...
namespace
{
    ...
    CNewUIButton s_BtnVoiceMicrophone;
    CNewUIButton s_BtnVoiceListening;
    ...
    constexpr float kCapExtra = 72.f;
...
bool CNewUIHeroPositionInfo::Create(CNewUIManager* pNewUIMng, int x, int y)
{
    ...
    MoveTextTipPos(&s_BtnVoiceMicrophone, -20, 9);
    MoveTextTipPos(&s_BtnVoiceListening, -20, 9);
...
bool CNewUIHeroPositionInfo::Render()
{
    ...
    const auto RenderVoiceButton = [](CNewUIButton& button, const wchar_t* icon, bool enabled)
    {
        button.Render();
        ...
        g_pRenderText->SetBgColor(0, 0, 0, 0);
        g_pRenderText->RenderText(pos.x, pos.y + 1, icon, 18, 11, RT3_SORT_CENTER);
    };
